...
Antes de passar para análise, é preciso informar que os dados de grupo prioritários: Publico Geral e Criança , foram retirados, já que o foco é o grupos de outras áreas. Podemos notar que a diferença de porcentagem dos grupos prioritários não são tão notaveis, contrariamente ao número total de vacinados entre esses anos. Demonstrando e se questionando se a prioridade que foram dadas a estes grupos, balanceia os não prioritários.
```{r echo=FALSE, message=FALSE, warning=FALSE}
v2021 <- vacinados %>% filter(vacinados$data_vacinacao > '2021-01-01' & vacinados$data_vacinacao < '2022-01-01')
v2022 <- vacinados %>% filter(vacinados$data_vacinacao > '2022-01-01' & vacinados$data_vacinacao < '2023-01-01')
...
Diferentemente do que é visto nas primeiras doses, é visto uma diminuição exorbitante no processo de vacinação da 4ºdose, números esses que apenas tendiam a diminuir, devido ao fato de não haver mais o "medo" que a COVID troxe, contudo a implementação das doses são se suma importância, saúde deve ser tratada como prioridade. 
Fato da 4ºdose ter sido minima, encontrasse 2 fatores que são válidos citar. Fato Nº1 -  Ela ainda estar em vigor.
Fato Nº2 - Descaso da população em procura da dose.
...
**Foram selecionadas apenas informações importantes para explorar, dados de vacinações, doses e grupos, todos foram abordados visando demonstrar estatistica referente à seus dados.**
